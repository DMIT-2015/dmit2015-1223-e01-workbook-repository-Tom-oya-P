...
public class EnforcementZoneCentreResource {
    ...
    public Response listEnforcementZoneCentres() {
        return Response.ok(_enforcementZoneCentreRepository.findAll()).build();
    }

    @Path("{id}")
    @GET    // This method only accepts HTTP GET requests.
    public Response findEnforcementZoneCentreById(@PathParam("id") Short siteId) {
        EnforcementZoneCentre existingEnforcementZoneCentre = _enforcementZoneCentreRepository.findById(siteId).orElseThrow(NotFoundException::new);
    ...
    public Response addEnforcementZoneCentre(EnforcementZoneCentre newEnforcementZoneCentre, @Context UriInfo uriInfo) {
        ...
        try {
            ...
            _enforcementZoneCentreRepository.add(newEnforcementZoneCentre);
        } catch (Exception ex) {
        ...
        URI location = uriInfo.getAbsolutePathBuilder()
                .path(String.valueOf(newEnforcementZoneCentre.getSiteId()))
                .build();
...
//        existingEnforcementZoneCentre.setVersion(updatedEnforcementZoneCentre.getVersion());
    ...
    public Response delete(@PathParam("id") Short id) {
        ...
        try {
            _enforcementZoneCentreRepository.delete(existingEnforcementZoneCentre);    // Removes the EnforcementZoneCentre from being persisted
        } catch (Exception ex) {
            // Return a HTTP status of "500 Internal Server Error" containing the exception message
            return Response
                    .serverError()
